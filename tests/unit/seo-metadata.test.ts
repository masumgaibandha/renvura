...
describe('route registry', () => {
  ...
  it('made search navigable in Phase 3 but never indexable', () => {
    // A search-results URL is generated by a customer, not authored — one
  ...
  it('keeps the shop out of the sitemap while its catalogue is being prepared', () => {
    // `/products` is built (Phase 2C) but not yet indexable: an empty shop, or
    // one showing protected demo records, must not be what Google sees.
    const products = siteRoutes.find((route) => route.path === '/products');

    expect(products?.phase).toBe('built');
    expect(products?.indexable).toBe(false);
    expect(sitemapRoutes.map((route) => route.path)).not.toContain('/products');
  });
